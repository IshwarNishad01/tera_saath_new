                ...
                <div class="bg-white rounded shadow-sm p-4 text-left">
                    ...
                    <form method="POST" action="{{ route('password.email') }}">
                        ...
                        <div class="form-group">
                            @if (addon_activation('otp_system'))
                                <input id="email" type="text" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required placeholder="test@gmail.com or +910000000000">
                            @else
