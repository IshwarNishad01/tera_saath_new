    ...
    <form action="{{ route('physical-attribute.update', $member->id) }}" method="POST">
        ...
        <div class="form-group row">
            <div class="col-md-6">
                ...
                <input type="number" name="height" value="{{ !empty($member->physical_attributes->height) ? $member->physical_attributes->height : "" }}" step="0.1" class="form-control" placeholder="{{translate('Height')}}" required>
                @error('height')
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="weight" value="{{ !empty($member->physical_attributes->weight) ? $member->physical_attributes->weight : "" }}" placeholder="{{ translate('Weight') }}" class="form-control" required>
                @error('weight')
                    <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
