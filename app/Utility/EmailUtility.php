...
class EmailUtility
{
    public static function account_oppening_email($user = '', $pass = '')
    {
    ...
    public static function email_on_express_interest($user = '' , $express_interest = '')
    {
        $subject    = get_email_template('email_on_express_interest','subject');
        $email_body = get_email_template('email_on_express_interest','body');
        $email_body = str_replace('[[name]]', $user->first_name.' '.$user->last_name, $email_body);
        $email_body = str_replace('[[member_name]]', Auth::user()->first_name.' '.Auth::user()->last_name , $email_body);
        $email_body = str_replace('[[from]]', env('MAIL_FROM_NAME'), $email_body);
